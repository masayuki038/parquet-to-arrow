...
public class FloatConverterTest extends ConverterTest {
    @Test
    public void floatConverterTest() throws IOException {
        ...
        for (int i = 0; i < vector.getValueCount(); i++) {
            assertThat(vector.getObject(i), is(1.0f + i));
        }
    }
    ...
    public FieldVectorConverter createConverter(Configuration conf, Path inPath, ParquetMetadata metaData, MessageType schema, List<ColumnDescriptor> columns) {
        ColumnDescriptor column = columns.get(TestParquetFileGenerator.FLOAT_FIELD_INDEX);
        return new FloatConverter(conf, metaData, schema, inPath, column);
    }
}
